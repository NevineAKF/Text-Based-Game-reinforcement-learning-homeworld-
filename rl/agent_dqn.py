...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        with torch.no_grad():
            q_actions, q_objects = model(state_vector)
        action_index = torch.argmax(q_actions).item()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # Q(s’,·)
    with torch.no_grad():
        q_actions_next, q_objects_next = model(next_state_vector)

    # max_a Q(s’, a)
    maxq_next = 0.5 * (q_actions_next.max() + q_objects_next.max())

    # Target y
    if terminal:
        target = torch.tensor([reward], dtype=torch.float32)
    else:
        target = torch.tensor([reward + GAMMA * maxq_next.item()],
                              dtype=torch.float32)

    # Q(s,a) for current state
    q_actions_cur, q_objects_cur = model(current_state_vector)

    # pick the predicted Q-value for chosen (action, object)
    q_pred = 0.5 * (q_actions_cur[action_index] +
                    q_objects_cur[object_index])
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        state_text = current_room_desc + current_quest_desc
        state_vector = torch.FloatTensor(utils.extract_bow_feature_vector(state_text, dictionary))

        action_index, object_index = epsilon_greedy(state_vector, epsilon)
        ...
        if for_training:
            deep_q_learning(state_vector, action_index, object_index, reward,
                            next_state_vector, terminal)
        else:
