...
def state_action_feature(state_vector, action_index, object_index):
    """
    Construct the feature vector φ(s,a) of size (NUM_ACTIONS * NUM_OBJECTS * d_R).

    State_vector: ψ_R(s)  (length d_R)
    We place this vector in the block corresponding to (action_index, object_index)
    and zeros elsewhere.
    """
    d_R = len(state_vector)
    total_actions = NUM_ACTIONS * NUM_OBJECTS

    # Create zero feature vector of size d_R * total_actions
    phi = np.zeros(d_R * total_actions)

    # Convert (action, object) pair → single index for block location
    idx = tuple2index(action_index, object_index)

    # Compute start and end positions for this block
    start = idx * d_R
    end = start + d_R

    # Insert state_vector into the correct block
    phi[start:end] = state_vector
...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    Args:
        state_vector (np.ndarray): vector representation ψ_R(s)
        theta (np.ndarray): weight matrix of shape (NUM_ACTIONS * NUM_OBJECTS, d_R)
        epsilon (float): exploration probability

    Returns:
        (int, int): (action_index, object_index)
    """
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    # Q(s,a)
    q_current = (theta @ current_state_vector)[idx]

    # target y
    if terminal:
        y = reward
    else:
        q_next_all = theta @ next_state_vector   # shape: (NUM_ACTIONS * NUM_OBJECTS,)
        y = reward + GAMMA * np.max(q_next_all)

    # feature φ(s,a) is just ψ_R(s)
    phi = current_state_vector   # shape: (d_R,)

    # gradient update: only row idx is updated
    theta[idx, :] += ALPHA * (y - q_current) * phi
